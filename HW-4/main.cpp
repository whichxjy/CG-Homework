...
struct Shader : public IShader {
    ...
    virtual Vec4f vertex(unsigned int index, int iface, int nthvert) {
        ...
        Vec4f gl_Vertex = Projection * ModelView *
                          embed<4>(models[index]->vert(iface, nthvert));
    ...
    virtual bool fragment(unsigned int index, Vec3f pos, Vec3f bar,
                          TGAColor& color) {
        ...
        float spec = 0.0f;
        // TODO 2:
        // calculate the specular coefficient using Phong model, and then save
        // it to spec.
        int p = 64;
        {
            Vec3f reflectedLightDir =
                (normal * (normal * lightDir) * 2 - lightDir).normalize();
            spec = pow(viewDir * reflectedLightDir, p);
        }
        // TODO 3:
        // calculate the specular coefficient using Blinn-Phong model, and then
        // save it to spec.
        {
            Vec3f half = (lightDir + viewDir).normalize();
            spec = pow(normal * half, p);
        }
...
int main(int argc, char** argv) {
    ...
    while (key != 27) {
        ...
        // TODO 4: rotate the eye around y-axis.
        {
            ...
            float angle = 20;
            float rotationRad = angle * MY_PI / 180;

            Matrix rotateMatrix = Matrix::identity();
            rotateMatrix[0][0] = cos(rotationRad);
            rotateMatrix[0][2] = sin(rotationRad);
            rotateMatrix[2][0] = -sin(rotationRad);
            rotateMatrix[2][1] = 1;
            rotateMatrix[2][2] = cos(rotationRad);

            Vec4f rotated = rotateMatrix * embed<4>(eye);
            eye[0] = rotated[0];
            eye[1] = rotated[1];
            eye[2] = rotated[2];
        }
